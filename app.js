...
async function loadData() {
    try {
        ...
        if (!db) {
            ...
            // Optionally alert the user or handle this state appropriately
            return;
        }
        ...
        usageRecords = usageSnapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data(),
            // Provide default for hours if missing or invalid
            hours: typeof doc.data().hours === 'number' ? doc.data().hours : 0
        }));
...
async function handleRecordPayment(e) {
    ...
    try {
        ...
        const newPaidAmount = (bill.paidAmount || 0) + paymentAmount; // Ensure paidAmount starts as 0 if undefined

        // Ensure status is a valid string before updating
        let newStatus = typeof bill.status === 'string' ? bill.status : 'Unpaid';

        if (newPaidAmount >= (bill.amount || 0)) { // Ensure bill.amount is treated as 0 if undefined
            newStatus = 'Paid';
...
function updateUI() {
    ...
    usageHistory.innerHTML = usageRecords.map(u => {
        ...
        const hoursDisplay = u.hoursDisplay !== undefined ? u.hoursDisplay : (u.hours ? Math.floor(u.hours) : 0);
        const minutesDisplay = u.minutesDisplay !== undefined ? u.minutesDisplay : (u.hours ? Math.round((u.hours % 1) * 60) : 0);
        return `
    ...
    billingHistory.innerHTML = bills.map(b => {
        ...
        const totalMinutes = b.totalMinutes !== undefined ? b.totalMinutes : (b.totalHours !== undefined ? Math.round(b.totalHours * 60) : 0); // Also handle older totalHours field
        const amount = b.amount !== undefined ? b.amount : totalMinutes; // Default amount based on minutes if missing
